...
def ruch(window, top, bottom, left, right):
    ...
    for i in range(len(atomy)):
        ...
        # if right < atomy[i].x or atomy[i].x < left:
        #     print(atomy[i].x)
        # if top < atomy[i].y < bottom:
        #     print(atomy[i].y)
        ...
        for j in range(i + 1, len(atomy)):
            ...
            if s > atomy[i].dystans(atomy[j]) and (i != atomy[j].bounce or j != atomy[i].bounce):
                print("grrr")
                while s > atomy[i].dystans(atomy[j]):
                    h=s-atomy[i].dystans(atomy[j])
                    print(s, atomy[i].dystans(atomy[j]))
                    print(s, atomy[i].dystans(atomy[j]))
            ...
            # Odbicie
            elif s < (atomy[i].dystans(atomy[j]) <= s + tol and (i != atomy[j].bounce or j != atomy[i].bounce)):
                if i == 0 or j == 0:
                    lambdy.append(
...
while True:
    ...
    print(atomy[91].x, atomy[91].y, atomy[84].x, atomy[84].y, atomy[91].dystans(atomy[84]))
    print("speed", atomy[91].speed_x, atomy[91].speed_y, atomy[84].speed_x, atomy[84].speed_y)
    draw_setup(screen)
    ...
    czas.tick(100)  # spowalnia, max 60 klatek na sekundę
